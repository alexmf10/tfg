...
## 2026-08-06 · Auditoría de traspaso previa a la migración de chat maestro

Auditoría de traspaso previa a la migración de chat maestro: 7 comprobaciones, 6 hallazgos (1 corregido mecánicamente, 5 elevados). Ejecutada por Claude Code como ejecutor suplente (D30): coherencia interna del plan (D1–D43 correlativas; D39–D43 vs §8/§9/§10/§14/§15; FC-01/FC-02; escalera {1,5,20}, batch 64 y LR de D40), estados residuales, bitácora en orden cronológico con append-only contrastado en git (13 commits sobre bitacora.md, 0 borrados), 231/231 valores finales (92/83/56) no vacíos con referencia de cierre y sin flags huérfanos (S/BHV/OBS), colas consolidada y por método cerradas con referencia y PD-01…04 en DECIDIDO, fuentes.md contrastado con la realidad (SHAs de l2p@dd8836e6, codaprompt@6417d4f6, papers y dosieres exactos; rama tfg-auditoria = e75a491 + dump_config.py + parche D39 confinado a models/coda_prompt.py + validador) y sincronización HEAD=origin con árboles limpios en TFG y mammothV2. Único arreglo mecánico: línea de estado de cabecera del plan («D40–D43 registradas y FC-01/FC-02 decididos y encolados»). Los cinco hallazgos elevados quedan detallados con literales propuestos en el parte de traspaso siguiente.

Evidencia: `PLAN_MAESTRO.md` cabecera y §13/§15; `git log --numstat -- bitacora.md`; `auditoria/valores_{l2p,dualprompt,coda}.md`; `auditoria/cola_revision.md`; `auditoria/cola_{l2p,dualprompt,coda}.md`; `auditoria/fuentes.md`; repos locales `l2p`, `codaprompt`, `mammothV2` (tfg-auditoria@4353bc1).

Estado: **RESUELTO**; los cinco elevados quedan a decisión del maestro tras la migración.

## Parte de traspaso — auditoría previa a la migración de chat maestro — 2026-08-06

**Estado del proyecto.** Fase B completada y verificada contra evidencia, lista para migrar el chat maestro. Registro D1–D43 correlativo y sin huecos; 231/231 valores finales escritos (L2P 92, DualPrompt 83, CODA 56) con referencia de cierre y sin flags huérfanos; colas cerradas con referencia y PD-01…04 en DECIDIDO; bitácora en orden cronológico y append-only real (verificado commit a commit); `fuentes.md` fiel a la realidad del workspace (SHAs de repos, papers y dosieres comprobados uno a uno; la rama `tfg-auditoria` contiene exactamente dump_config.py, el parche D39 confinado a `models/coda_prompt.py` y el validador); HEAD=origin y árboles limpios en ambos repos. FC-01 y FC-02 decididos y encolados para su hueco de Fase C/D.

**Corregido mecánicamente (1).** La línea de estado de la cabecera del plan decía «D40–D42 registradas y FC-01 decidido y encolado»; los commits bfa9887/d83a554/3e67da3 (5–6 ago) registraron D43 y FC-02 sin actualizarla. Corregida a «D40–D43 registradas y FC-01/FC-02 decididos y encolados». La entrada datada [5 ago] de §15 no se toca: es registro histórico exacto de su momento.
...
## 2026-08-07 · Análisis estático integral pre-Acta (FASE 0-3)

Bloque D30 en suplencia (Claude Code, nivel muy alto): análisis estático de dependencias ocultas en mammothV2 (tfg-auditoria@4353bc1) y re-auditoría de las 231 filas de `valores_*.md` en dos profundidades, sin modificar ningún fichero existente ni ejecutar nada en GPU. Entregable nuevo: `auditoria/analisis_dependencias.md` (inventario FASE 1 con file:línea; pasada mecánica 231/231 — test de cubo, coherencia caso↔evidencia, configurabilidad con flag exacto —; pasada profunda sobre los 16 CONFLICTO→PAPER, las 4 SIN_FUENTE y las filas S/grupo, contra LaTeX congelado y repos pineados). Resultado: 0 CONTRADICE-VALOR, 0 CONTRADICE-CUBO, 0 ELEVADA-AL-ACTA; 8 TRAMPA-CONFIG (2 nuevas en su consecuencia operativa: `--dataset_config` explícito obligatorio al estar prohibido `model_config best` en bloque, y `--permute_classes 1` imprescindible para el «orden por semilla» de D19); 3 DEPENDENCIA-NO-REGISTRADA (olvido F_N exige `--enable_other_metrics 1` o cálculo offline; `optim_mom` de CODA inerte con Adam; cableado scheduler asimétrico sin efecto); 74 filas NO-CONFIGURABLE confirmadas sin falsos positivos; resto CONFIRMADO (fórmula coseno Mammoth ≡ oficial, semántica de step D39 ≡ oficial, errata de betas de CODA verificada en `7_appendix.tex:5`, split Pre-T verificado en `dual_prompt_camera_ready.tex:405-409`, solo LayerNorm en los tres caminos).

Evidencia: `auditoria/analisis_dependencias.md` (localizadores completos); mammothV2 `main.py:177-186`, `models/l2p.py:66`, `models/dualprompt.py:50,71`, `models/coda_prompt.py:52-63,73-90`, `datasets/utils/continual_dataset.py:228-232`, `utils/args.py:382-383`; `l2p/configs/cifar100_{l2p,dualprompt}.py`; `codaprompt/configs/cifar-100_prompt.yaml` y `utils/schedulers.py:53-54`.

Estado: **RESUELTO** (el bloque describe; las 2 trampas operativas nuevas y la dependencia del olvido quedan consignadas en §3.3 del entregable para el Acta de configuración de Fase D).

## Parte de novedades — análisis estático integral pre-Acta — 2026-08-07

**Commit base al abrir el bloque:** TFG master `392aab9` «Resuelve los elevados de la auditoría de traspaso (visto bueno 6-ago)»; mammothV2 tfg-auditoria `4353bc1` «Fix D39 validator logging startup»; ambos árboles limpios y HEAD=origin (FASE 0 conforme).

- **Entregable único nuevo:** `auditoria/analisis_dependencias.md`. §1 inventario FASE 1 (batch-dependencias, mutaciones runtime, cadena de precedencia real con el mapa de valores silenciosos que difieren del pre-final, propagación de semilla, enmascarado de evaluación — todo con file:línea). §2 re-auditoría: mecánica 231/231 y profunda con extractos primarios. §3 clasificación y recuentos.
- **Recuentos:** CONTRADICE-VALOR 0 · CONTRADICE-CUBO 0 · TRAMPA-CONFIG 8 · DEPENDENCIA-NO-REGISTRADA 3 · NO-CONFIGURABLE 74 filas · CONFIRMADO 231/231 · ELEVADA-AL-ACTA 0.
- **Lo que el Acta debe recoger (§3.3 del entregable):** (1) `configuracion_final` con `--dataset_config l2p|coda_prompt` explícito — sin él, la tubería cae en silencio a `default.yaml` con normalización ImageNet; (2) `--permute_classes 1` junto a `--seed` — la semilla sola no permuta clases y D19 quedaría incumplida; (3) decisión sobre `--enable_other_metrics 1` u olvido calculado offline por el script de agregación; (4) el resto de trampas (batchwise, length 20 de Dual, LR nominales D40, batch 64, n_epochs) ya estaban registradas en tablas/decisiones y solo exigen que los flags de §1.3 viajen a la configuración final.
- **Sin cambios en:** plan, tablas, colas, comportamiento_*, entorno, piloto0, código de mammothV2. Cero ediciones; bitácora append-only con 0 borrados.
- **Commit:** único, «Análisis estático integral pre-Acta», sobre `392aab9`; su hash no consta aquí (regla del parte) y es verificable como HEAD de `origin/master` inmediatamente posterior.
