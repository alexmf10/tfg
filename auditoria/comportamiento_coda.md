...
3. **Configurable en Mammoth.** Selección `model_config` sí; el override de `dataset_config` por el modelo forma parte de la resolución. La interpolación no está fijada por CLI: el YAML la delega a torchvision. La Fase B la resuelve para `galatzo`/torchvision 0.17.1 como bilineal y antialias tanto en RRC train como en Resize test, con escala `(0.08,1.0)` y ratio `(0.75,1.3333)` en train (`methods[2].dataset_effective`, hashes en `reconciliacion_fase_b.md`).
